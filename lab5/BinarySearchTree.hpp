...
public:
  ...
  //lab6
  void iterativeBreadth(bool isPrinting = true);//in width walk
  std::string iterativeInorder(bool isPrinting = true);//left to right walk
  bool sameTrees(BinarySearchTree<T> src);
...
template<class T>
void BinarySearchTree<T>::iterativeBreadth(bool isPrinting)
{
  Node* ptrRoot = root_;
  QueueList<Node*> queue;
  queue.enQueue(ptrRoot);
  while(!queue.isEmpty())
  {
...
template<class T>
std::string BinarySearchTree<T>::iterativeInorder(bool isPrinting)
{
  Node* current = root_;
  std::string result = "";
  StackList<Node*> stack;
...
template<class T>
bool BinarySearchTree<T>::sameTrees(BinarySearchTree<T> src)
{
  const std::string tree1 = this->iterativeInorder(false);
  const std::string tree2 = src.iterativeInorder(false);
  return !strcmp(tree1.c_str(), tree2.c_str());
}
